...
- `backtest_gui.py` offers a small graphical interface to run the backtests.
...
Backtest a strategy from the command line (e.g. EMA breakout) with:

```bash
python backtest.py --strategy breakout_ema_volume --symbol BTCUSDT --interval 30m
```

Launch the GUI to select strategies interactively:
